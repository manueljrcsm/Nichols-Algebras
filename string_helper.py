...
def string_compressor(input_string: str):
    """Compresses a string of letters into a factorized expression with powers.

    E.g., string_compressor("abcabababbaabba") = abc(ab)²(ab²a)². """
    ...
    while i <= len(input_string) - max_len:
        ...
        while j > 0:
            ...
            if input_string[i:i + j] == input_string[i + j:i + j + j]:
                while (i + (cur_mult + 1)*j <= len(input_string) and input_string[i:i + j] == input_string[
                                                                                              i + cur_mult*j:i + (
                                                                                                      cur_mult + 1)*j]):
                    cur_mult += 1
    ...
    if max_len == 1 and max_mult == 1:
        ...
        if max_len == 1:
            output_string += (string_compressor(input_string[max_index:max_index + max_len]) + power_escape(max_mult))
        else:
            output_string += ("(" + string_compressor(input_string[max_index:max_index + max_len]) + ")" + power_escape(
                max_mult))
