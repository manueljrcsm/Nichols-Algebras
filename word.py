...
class Word:
    ...
    def __add__(self, other):
        """
            To prevent ambiguity: Addition of words means concatination.
        """
        if type(other) != Word:
            msg = ("Two incompatible types, "+ str(type(self)) +
            " and "+ str(type(other))+ " were tried to be added.")
            raise AssertionError(msg)
    ...
    def coproduct(self):
        """
            ...
            cop(ab....x)= 1@1 * cop(a)* cop(b) *... cop(x)
        """
    ...
    def c_bilinear(self, other):
        """
            Computes the c bilinear form of a word and another word.
            Basic idea is to use the compatibility of (co-products with 
            the c bilinear to reduce the word length.
            Roughly the process is:
                word A = ab...c, word B = uv....x
                Step one: reduce word A until it has length 1 i.e. is a letter.
                    Beware that this changes word B into a tensor element T
                    consisting of tensorands B(1) +.....+ B(n)
        ...
        if self.length > 1:
            for term, sca in other.coproduct().pairs:
                output_number += (sca* 
                                  Word([self.letters[0]]).c_bilinear(term.words[0])*
                                  Word(self.letters[1:]).c_bilinear(term.words[1]))
        #Step 2: Reduce the length of other
        elif other.length > 1:
            for term, sca in self.coproduct().pairs:
                output_number += (sca* 
                                  term.words[0].c_bilinear(Word([other.letters[0]]))*
                                  term.words[1].c_bilinear(Word(other.letters[1:])))
    ...
    def stats_string(self):
        ...
        output = ("This is the word " + str(self) + 
                  ". Its length is " + str(self.length) + ". "
                  "It has the degrees "+ 
                  ",".join( str(key)+": "+str(val) for key, val in self.degree.items()) + ".")
        return output
...
class TensorWord:
     ...
     def __init__(self, word_list):
        try:
            ...
            if (all(type(w) is Word for w in word_list)):
                ...
                msg = ("Please make sure to initialize this class "+ 
                       "with a list of words as parameter .")
                raise AssertionError(msg)
     ...
     def __str__(self):
         ...
         return (u'\u2297').join(
            str(self.words[i]) if (i==0 or not self.words[i].is_unit()) else "1" 
            for i in range(self.tensor_degree)
            )
        
     def __add__(self, other):
         
        if(self.tensor_degree != other.tensor_degree):
            msg = ("You tried to simulanous concatenate elements "+
                   "of different tensor degrees. This is not supported.")
            raise AssertionError(msg)   
     ...
     def __eq__(self, other):
        ...
        if type(other) != TensorWord: 
            ...
            return (self.tensor_degree == other.tensor_degree and
            all(self.words[i] == other.words[i] for i in range(self.tensor_degree)))            
     ...
     def coproduct(self):
        """
                Computes the coproduct of a given TensorWord 
                by applying the coproduct to its first term
        """    
        #Case 1: The tensor word is just a word
        if self.tensor_degree == 1:
             return self.word_list[0].coproduct()
